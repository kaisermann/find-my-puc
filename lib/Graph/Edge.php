...
class Edge extends Entity
{
	...
	public function printMe($identation = 0)
	{
		...
		echo sprintf("%sNODE A: %s - %s<br />", str_repeat("--", $identation + 2), $this->nodeA->getAttr(NODE_UNIQUE_ID), $this->nodeA->getAttr(NODE_NAME_ID));
		echo sprintf("%sNODE B: %s - %s <br />", str_repeat("--", $identation + 2), $this->nodeB->getAttr(NODE_UNIQUE_ID), $this->nodeB->getAttr(NODE_NAME_ID));
